...
const std::vector<int> symmetry_value {5, 8, 10, 12};
...
iterfunc randfunc_p31m()
{
  if ( random_bool() ) {
    ...
    return [ = ] ( double x, double y ) {
      ...
      return tr( make_tuple( a1 * x + a3 * ( ey - ez ).real() + a4 * ( 2.*ex - ey - ez ).imag() + a5 * x1.real() + a6 *
                             x1.imag(),
...
iterfunc randfunc_p3m1()
{
  if ( random_bool() ) {
    ...
    return [ = ] ( double x, double y ) {
      ...
      return tr( make_tuple( a0 + a1 * x + a3 * x1.real() + a4 * x1.imag() + a5 * ( ezx - exy ).imag() + a6 *
                             ( 2.*eyz - exy - ezx ).real(), a0 + a1 * y + a3 * y1.real() + a4 * y1.imag() + a5 * ( exy - eyz ).imag() + a6 *
                             ( 2.*ezx - exy - eyz ).real() ) );
    };
  } else {
    double a0 = random_angle(), b0 = random_angle();
...
iterfunc randfunc_p4m()
{
  if ( random_bool() ) {
    ...
    int a1 = random_range_inclusive( -1, 1 );
    double a3 = random_normal( 1. ), a4 = random_normal( 1. ), a5 = random_normal( 1. );
    auto fn = PT( a0 + a1 * x + a3 * sx + a4 * sx * cy + a5 * sx * cx,
                  a0 + a1 * y + a3 * sy + a4 * sy * cx + a5 * sy * cy );
...
iterfunc randfunc_p4g()
{
  ...
  double a3 = random_normal( 1. ), a4 = random_normal( 1. ), a5 = random_normal( 1. );
  auto tr = random_choice( p4m_trans );
  auto fn = PT( a0 + a1 * x + a3 * sy + a4 * sx * cx + a5 * sx * cy,
                a0 + a1 * y - a3 * sx + a4 * sy * cy + a5 * sy * cx );
  return [ = ] ( double x, double y ) { return tr( fn( x, y ) ); };
}
...
void drawtrap( ImageData& imageData, int colRed, int colGreen, int colBlue, int sz, int symGroup )
{
  ...
  imageData.img =  std::make_shared<symmetric_canvas<color_t>>( c );
}
...
qiterfunc random_qiterfunc5()
{
  ...
  return [ = ] ( double x, double y, double z, double w ) {
    ...
    double xn = a0 + ( a3 * ( ex - ey ) + a4 * ( ey - ez ) + a5 * ( ez - ew ) + a6 * ( ew - ev ) + a7 *
                       ( ev - ex ) ).real();
    double yn = a0 + ( a7 * ( ex - ey ) + a3 * ( ey - ez ) + a4 * ( ez - ew ) + a5 * ( ew - ev ) + a6 *
                       ( ev - ex ) ).real();
    double zn = a0 + ( a6 * ( ex - ey ) + a7 * ( ey - ez ) + a3 * ( ez - ew ) + a4 * ( ew - ev ) + a5 *
                       ( ev - ex ) ).real();
    double wn = a0 + ( a5 * ( ex - ey ) + a6 * ( ey - ez ) + a7 * ( ez - ew ) + a3 * ( ew - ev ) + a4 *
                       ( ev - ex ) ).real();
                         ...
                         int symmetryValueIndex, double quasiperiod )
{
  ...
  int symmetryValue = symmetry_value[size_t( symmetryValueIndex ) ];
  qpiterfunc f;
  qembfunc e;
  qpdistfunc d;
  ...
  if ( symmetryValue == 8 ) {
    ...
    e = qembfunc8;
    d = qpdistfunc8;
  } else if ( symmetryValue == 10 ) {
    f = random_qpiterfunc10();
    e = qembfunc5;
    d = qpdistfunc5;
  } else if ( symmetryValue == 12 ) {
