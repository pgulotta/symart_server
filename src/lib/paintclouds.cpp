...
const std::vector< clouds_randfunc> randfuncs = { &clouds_rand_cauchy, &clouds_rand_normal, &clouds_rand_exp_cos, &clouds_rand_sechsquare};
...
color_t random_color( color_t c1, color_t c2, int d, clouds_randfunc r )
{
  return color_t( random_byte( c1.red, c2.red, d, r ),
                  random_byte( c1.green, c2.green, d, r ),
                  random_byte( c1.blue, c2.blue, d, r ) );
}
...
void copy( symmetric_canvas<color_t>& img,
           canvas<color_t>& tri,
           int x1,
           int y1,
           int x2,
...
canvas<color_t> tri_a222( int sz, color_t col1, color_t col2, color_t col3, clouds_randfunc r )
{
  int d = tri_size( sz );
  canvas<color_t> tri( d + 1, d + 1 );
  tri( 0, 0 ) = tri( d, 0 ) = color_t( ( ( int ) col1.red + col2.red + col3.red ) / 3,
                   ...
                   int col3Red, int col3Green, int col3Blue, int distributionIndex )
{
  ...
  color_t col3{col3Red, col3Green, col3Blue};
  symmetric_canvas<color_t> grid( size, symGroup );
  canvas<color_t> tri;
  int halfsize = size / 2;

  switch ( symGroup ) {
  case symgroup::CM:
    tri = tri_sax( size, col1, col2, col3, randfunc );
    copy( grid, tri, size, 0, size, size, 0, 0 );
    break;

  case symgroup::CMM:
    tri = tri_2sab( size, col1, col2, col3, randfunc );
    copy( grid, tri, halfsize, halfsize, 0, 0, size, 0 );
    break;

  case symgroup::P1:
    tri = quad_ao( size, col1, col2, col3, randfunc );
    copy( grid, tri, 0, 0, 0, size, size, 0, 1, false );
    copy( grid, tri, size, size, size, 0, 0, size, 1, true );
    break;

  case symgroup::P2:
    tri = tri_a222( size, col1, col2, col3, randfunc );
    copy( grid, tri, 0, 0, size, 0, 0, size );
    break;

  case symgroup::P3:
    tri = quad_abc( ( 2 * size + 1 ) / 3, col1, col2, col3, randfunc );
    copy( grid, tri, 0, 0, size, size, 2 * size, -size, 3, false );
    copy( grid, tri, 0, 0, -size, 2 * size, size, size, 3, true );
    break;

  case symgroup::P31M:
    tri = tri_asb( size, col1, col2, col3, randfunc );
    copy( grid, tri, size, size, 3 * size, 0, 0, 3 * size, 3 );
    break;

  case symgroup::P3M1:
    tri = tri_sabc( ( 2 * size + 1 ) / 3, col1, col2, col3, randfunc );
    copy( grid, tri, 0, 0, size, size, 2 * size, -size, 3 );
    break;

  case symgroup::P4:
    tri = tri_ab2( size, col1, col2, col3, randfunc );
    copy( grid, tri, halfsize, halfsize, size, 0, 0, 0 );
    break;

  case symgroup::P4G:
    tri = tri_asb( halfsize, col1, col2, col3, randfunc );
    copy( grid, tri, 0, 0, 0, halfsize, halfsize, 0 );
    break;

  case symgroup::P4M:
    tri = tri_sabc( halfsize, col1, col2, col3, randfunc );
    copy( grid, tri, 0, 0, halfsize, 0, halfsize, halfsize );
    break;

  case symgroup::P6:
    tri = tri_ab2( ( 2 * size + 1 ) / 3, col1, col2, col3, randfunc );
    copy( grid, tri, 0, 0, size, size, 2 * size, -size, 3 );
    break;
                              ...
                              int col3Red, int col3Green, int col3Blue, int distributionIndex )
{
  ...
  switch ( fdfIndex ) {
  case 0:
    tri = tri_sax( trisize, col1, col2, col3, randfunc );
    break;

  case 1:
    tri = tri_2sab( trisize, col1, col2, col3, randfunc );
    break;

  case 2:
    tri = tri_a222( trisize, col1, col2, col3, randfunc );
    break;

  case 3:
    tri = tri_ab2( trisize, col1, col2, col3, randfunc );
    break;

  case 4:
    tri = tri_asb( trisize, col1, col2, col3, randfunc );
    break;

  case 5:
    tri = tri_sabc( trisize, col1, col2, col3, randfunc );
    break;

  case 6:
    tri = tri_a2x( trisize, col1, col2, col3, randfunc );
    break;

  case 7:
    tri = tri_22sa( trisize, col1, col2, col3, randfunc );
    break;

  case 8:
    tri = quad_asbc( trisize, col1, col2, col3, randfunc );
    break;

  case 9:
    tri = quad_sabcd( trisize, col1, col2, col3, randfunc );
    break;

  case 10:
    tri = quad_a2sb( trisize, col1, col2, col3, randfunc );
    break;

  case 11:
    tri = quad_sasb( trisize, col1, col2, col3, randfunc );
    break;

  case 12:
    tri = quad_ao( trisize, col1, col2, col3, randfunc );
    break;

  case 13:
    tri = quad_abc( trisize, col1, col2, col3, randfunc );
    break;

  case 14:
    tri = quad_axx( trisize, col1, col2, col3, randfunc );
    break;

  case 15:
    tri = quad_2sabc( trisize, col1, col2, col3, randfunc );
    break;

  case 16:
    tri = quad_abx( trisize, col1, col2, col3, randfunc );
    break;

  case 17:
    tri = quad_ab22( trisize, col1, col2, col3, randfunc );
    break;
  ...
  imageData.img =  std::make_shared<canvas<color_t>>( copy_hyperbolic( tri, pt, size, fd, trans,
                                                                       interpolate_closest ) );
}
