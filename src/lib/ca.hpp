...
public:
  ...
  {
    int sum = corner_weight * ( c.get( x - 1, y - 1 ) + c.get( x + 1, y - 1 ) + c.get( x - 1, y + 1 ) + c.get( x + 1,
                                y + 1 ) )
...
{
public:
  ca( int w, int h, int ruleindex ) : current( w, h ), old( w, h ),
    in_queue( w, h ), first_on( w, h ), next_on( w, h ), turn( 1 ), therule( *lookup_rulefunc( ruleindex ) ) {}
  void set( int x, int y, int s );
  void random_rectangle( int l, int t, int r, int b );
  canvas<color_t> generate_image( int nturns, double intensity );
private:
