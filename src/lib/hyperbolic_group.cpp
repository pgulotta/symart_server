...
const  std::vector<std::vector<fundamental_domain_family::side>> supported_groups {
...
tuple<hyperbolic_coord, bool> hyperbolic_symmetry_group::fundamental_domain_alternating(
  const hyperbolic_coord& hc ) const
{
...
const std::vector<std::vector<fundamental_domain_family::side>>& hyperbolic_supported_groups()
{
  return supported_groups;
}

const fundamental_domain_family fundamentalDomainFamily( int index )
{
  return supported_groups[index];
}


void extractRotations( const int& rotation0, const int& rotation1, const int& rotation2, const int& rotation3,
                       std::vector<int>& rotations )
{
  rotations.clear();

  if ( rotation0 != INVALID_VALUE ) {
    rotations.push_back( rotation0 );
  }

  if ( rotation1 != INVALID_VALUE ) {
    rotations.push_back( rotation1 );
  }

  if ( rotation2 != INVALID_VALUE ) {
    rotations.push_back( rotation2 );
  }

  if ( rotation3 != INVALID_VALUE ) {
