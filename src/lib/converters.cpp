...
coord_transformer triangle_transformer( const hyperbolic_polygon& t, double x1, double y1, double x2, double y2,
                                        double x3, double y3 )
{
...
coord_transformer quadrilateral_transformer( const hyperbolic_polygon& q, double x1, double y1, double x2, double y2,
...
void make_hyperbolic( ImageData& imageData, int projType, int newsize )
{
  projtype pt = static_cast<projtype>( projType );
  const symmetric_canvas<color_t>* img = imageData.img.sym_view;
  imageData.layers.reset();
  imageData.img =  std::make_shared<canvas<color_t>>( make_hyperbolic( *img, pt, newsize ) );
}
...
canvas<color_t> make_hyperbolic( const symmetric_canvas<color_t>& img, projtype pt, int newsize )
{
  ...
  case symgroup::PMG:
    ...
    fd = fdf.domain( {3, 1} );
    trans = quadrilateral_transformer( fd.polygon, 0, .5 * size, .25 * size, .75 * size, .25 * size, -.25 * size,
                                       -.25 * size, .25 * size );
    break;
...
void load_image( const QImage& source, int symGroup,  ImageData& destination )
{
  std::shared_ptr<symmetric_canvas<color_t>>  image = std::make_shared<symmetric_canvas<color_t>>( source.height(),
                                                      static_cast<symgroup>( symGroup ) ) ;

  canvas<color_t>& base_image = image->unsafe_get_canvas();

  for ( int j = 0; j < source.height(); j++ )
    for ( int i = 0; i < source.width(); i++ ) {
      QRgb pix = source.pixel( i, j );
      base_image( i, j ) = color_t( qRed( pix ), qGreen( pix ), qBlue( pix ) );
    }

  destination.layers.reset();
  destination.img =  std::make_shared<canvas<color_t>>( base_image );
}
