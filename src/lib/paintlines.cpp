...
tuple<int, int, int, int> random_endpoints( int size, int wrap = 2 )
{
  ...
  return make_tuple( i, j, i + random_range_inclusive( -wrap, wrap ) * size, j + random_range_inclusive( -wrap,
                     wrap ) * size );
}
...
{
  paintlines_layer_generator( g ).drawfractal( random_int( g.size() ), random_int( g.size() ), g.size(),
                                               fractal_prob[num_symmetries( g.group() )] );
}
