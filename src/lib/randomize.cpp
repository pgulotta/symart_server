...
typedef function<void( const vector<tuple<int, int>> &, const painter_transformation&, const painter_transformation& )>
copy_func_t;
...
void fill( int ke, int le, int kc, int lc, const function<bool( int, int )>& mark,
           const function<tuple<int, int>( int, int, int, int )>& mov, const function<void( int, int, int, int )>& put )
{
  while ( mark( ke, le ) ) {
    put( ke, le, kc, lc );
...
{
  copy_func_t copy_func = [&]( const vector<tuple<int, int>>& v, const painter_transformation & from_trans,
  const painter_transformation & to_trans ) {
    int xt, yt, xf, yf;
