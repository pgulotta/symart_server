...
canvas<uint8_t> paint_hyperbolic_lines( const hyperbolic_symmetry_group& sg,
                                        const hyperbolic_lines_param& params )
{
                             ...
                             int rotation3, int projType, int flipType, float thickness, float sharpness, int ncolors )
{
  ...
  hyperbolic_symmetry_group sg( fd.generators, static_cast<flip_type>( flipType ) );
  const hyperbolic_lines_param params{size,  5.0f * thickness,  5.0f * thickness * thickness, sharpness, static_cast<projtype>( projType )};
  imageData.layers = std::make_shared<vector<layer>>( ncolors );

  //    #pragma omp parallel for
  for ( size_t i = 0; i < static_cast<size_t>( ncolors ) ; i++ ) {
    ( *imageData.layers )[i].pixels  = std::make_shared<canvas<uint8_t>>( paint_hyperbolic_lines( sg, params ) );
    ( *imageData.layers )[i].color = default_color_generator();
    ( *imageData.layers )[i].pastel = false;
  }

  canvas<color_t> img( size, size );
  merge( img, *imageData.layers );
  imageData.img =  std::make_shared<canvas<color_t>>( img );
}
