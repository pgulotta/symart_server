...
const  std::vector<const rule*> rulefunc {
  &rule::threefour,
  &rule::banners,
  &rule::beams,
  &rule::blob,
...
const rule* lookup_rulefunc( int index )
{
  return  rulefunc[( size_t )index];
}
...
canvas<color_t> ca::generate_image( int nturns, double intensity )
{
  run_for( nturns );
  return get_diff_image( intensity );
}
...
canvas<color_t> ca::get_diff_image( double intensity )
{
  ...
  for ( int y = 0; y < first_on.height(); y++ )
    for ( int x = 0; x < first_on.width(); x++ ) {
      int xd = first_on.get_nearest( x + 1, y - 1 ) + 2 * first_on.get_nearest( x + 1, y ) + first_on.get_nearest( x + 1,
               y + 1 )
               - first_on.get_nearest( x - 1, y - 1 ) - 2 * first_on.get_nearest( x - 1, y ) - first_on.get_nearest( x - 1, y + 1 );
      int yd = first_on.get_nearest( x - 1, y + 1 ) + 2 * first_on.get_nearest( x, y + 1 ) + first_on.get_nearest( x + 1,
               y + 1 )
